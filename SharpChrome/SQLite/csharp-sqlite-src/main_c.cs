...
static int sqlite3ParseUri(
  ...
  if( ((flags & SQLITE_OPEN_URI) != 0 || sqlite3GlobalConfig.bOpenUri) 
    ...
    ** method that there may be extra parameters following the file-name.  */
    ...
    // ported SQLITE_ALLOW_URI_AUTHORITY logic from SQLite Release 3.8.0.2 On 2013-09-03
    //  -@harmj0y
    if( String.Compare( "///", zUri.Substring( 5, 3 )) == 0 ){
      ...
      ** common error, we are told, so we handle it as a special case. */
      if(String.Compare("///", zUri.Substring(7, 3)) == 0) { iIn++; Console.WriteLine("iIn2: {0}", iIn); }
