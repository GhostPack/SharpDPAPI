...
{
    /// <summary>
    /// Exports logins to an in-memory collection and not Console output or a CSV file.
    /// </summary>
    public class LoginsExport : ICommand
    {
        public static string CommandName => "loginsexport";
        ...
        public void Execute(Dictionary<string, string> arguments)
        {
            ...
            if (arguments.ContainsKey("/target")) {
                ...
                if (arguments.ContainsKey("/server") && (masterkeys.Count == 0)) {
                    Console.WriteLine(
                        "[!] Warning: the '/server:X' argument must be used with '/pvk:BASE64...', '/password:X' , or masterkey specification for successful decryption!");
                }

                Chrome.TriageChromeLogins(masterkeys, server, target, displayFormat, showAll, unprotect, stateKey,
                    browser, quiet);
            }
        }
    }
